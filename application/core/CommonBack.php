...
class CommonBack extends MX_Controller {
  ...
  public function __construct(){
    ...
    public function check_admin_user_session(){
        ...
        $allowed_pages = array('create','update','check_unique_email_login'); //these pages/methods do not require user authentication
        $allowed_control = array('login'); //methods of this controller does not require authentication
        $current_control = $this->router->fetch_class(); // get current controller, class = controller
        
        if(!is_admin_logged_in() && (in_array($page_slug,$allowed_pages)) && (in_array($current_control,$allowed_control))){
            return TRUE; //session is empty and pages is not restricted
        }else{

            //either page is resticted or session exist
            if(!is_admin_logged_in()){
                redirect('admin/login/create'); //redirect to home/login if session not exit
            }
            ...
            if(empty($page_slug)){
             ...
             $after_auth = array('create','update');  //restrict access to these pages if session is set
     ...
     * Added in ver 2.0
     */
    public function logout($is_redirect=TRUE){
